...
const WEATHER_MACRO_SCHEMA = "unnatural-weather-v1";
...
function normalizeSeason(season) {
  ...
  const map = {
    весна: "spring", spring: "spring",
    лето: "summer", summer: "summer",
    осень: "autumn", autumn: "autumn", fall: "autumn",
...
async function rollDie(sides) {
  if (globalThis.Roll) {
    const roll = await new Roll(`1d${sides}`).evaluate({ async: true });
    return roll.total;
  }
...
async function resolveChange(changeIndex) {
  ...
  const rolls = [];
  if (text.includes("[[D12]]")) {
    const d12 = await rollDie(12);
    rolls.push(`d12=${d12}`);
    text = text.replace("[[D12]]", String(d12));
  }
  if (text.includes("[[D6]]")) {
    const d6 = await rollDie(6);
    rolls.push(`d6=${d6}`);
    text = text.replace("[[D6]]", String(d6));
  }
  return { text, rolls };
}

function buildWeatherCard(result) {
  const curse = result.curse
    ? `<section><h3>Модификатор проклятия: ${escapeHtml(result.curse.name)} <small>(d6=${result.curse.roll})</small></h3><p>${escapeHtml(result.curse.text)}</p></section>`
    : "";
  const change = result.change
    ? `<section><h3>Когда переменится погода <small>(d6=${result.change.roll}${result.change.extraRolls.length ? `; ${escapeHtml(result.change.extraRolls.join(", "))}` : ""})</small></h3><p>${escapeHtml(result.change.text)}</p></section>`
    : "";
  return `
    <div class="aeriya-content aeriya-weather-card">
      <h2>Неестественная погода: ${escapeHtml(result.season.label)}</h2>
      <section>
        <h3>Погодное явление <small>(d6=${result.weather.roll})</small></h3>
        <p>${escapeHtml(result.weather.text)}</p>
      </section>
      ${change}
      ${curse}
      <p><em>Эффекты не применяются автоматически. Мастер трактует последствия по сцене, маршруту и состоянию отряда.</em></p>
    </div>`;
}
...
async function rollUnnaturalWeather(options = {}) {
  ...
  if (seasonMode === "random") {
    seasonRoll = await rollDie(4);
    seasonKey = ["spring", "summer", "autumn", "winter"][seasonRoll - 1];
  }
  ...
  let curse = null;
  if (includeChange) {
    const changeRoll = await rollDie(6);
    const resolved = await resolveChange(changeRoll);
    change = { roll: changeRoll, text: resolved.text, extraRolls: resolved.rolls };
  }
  if (includeCurse) {
    const curseRoll = await rollDie(6);
    const curseEntry = CURSE_TABLE[curseRoll - 1];
    curse = { roll: curseRoll, ...curseEntry };
  }
  const result = {
    source: "Генератор Неестественной Погоды",
    season: { key: seasonKey, label: season.label, roll: seasonRoll },
    weather: { roll: weatherRoll, text: season.entries[weatherRoll - 1] },
    change,
    curse
  };
  const content = buildWeatherCard(result);
  const chatData = {
    user: game.user?.id,
    speaker: ChatMessage.getSpeaker({ alias: "Погода Аэрии" }),
    content,
    flags: { [MODULE_ID]: { macroId: WEATHER_MACRO_ID, result } }
  };
...
async function ensureWeatherMacro() {
  ...
  const command = `game.aeriya.rollUnnaturalWeather({ includeChange: true, includeCurse: true });`;
  const data = {
...
Hooks.once("ready", () => {
  ...
  game.aeriya.ensureWeatherMacro = () => ensureWeatherMacro();
