...
const BOOK_SCENES_SCHEMA = "book-scenes-v3";
const BOOK_SOURCE = "Города и поселения Аэрии";

const ROWS = `Гравен Холлоу|Aeria Core / Сцены / Срединные Земли / Города|middle-lands|common|city|крупный город
...
function specs() {
  return ROWS.trim().split("\n").map((line) => {
    ...
    return { name, folder, shard, region, type, bookType, sourcePath: `book:goroda-i-poseleniya-aerii/${slug(folder)}/${slug(name)}`, description: `${name} — ${bookType}. Источник: книга «${BOOK_SOURCE}».` };
  });
}
...
function sceneData(spec, folder, existing) {
  const data = {
    ...
    flags: { [MODULE_ID]: { sourcePath: spec.sourcePath, documentKind: "scene", shard: spec.shard, region: spec.region, locationType: spec.type, bookType: spec.bookType, bookSource: BOOK_SOURCE, description: spec.description, displayMode: "show-only-city-image", tacticalMap: false, playerMovement: false, importSchema: BOOK_SCENES_SCHEMA } }
  };
...
async function importBookScenes({ overwrite = true, notify = true } = {}) {
  ...
  if (notify) {
    if (result.failed.length) {
      ...
      ui.notifications?.info(`Aeria Core: книжные сцены проверены. Создано: ${result.created}, обновлено: ${result.updated}, пропущено: ${result.skipped}.`);
    }
  }
  return result;
}
...
async function repairBookScenes() {
  ...
  ui.notifications?.info(`Aeria Core: сцены восстановлены из книги. Создано: ${imported.created}, обновлено: ${imported.updated}.`);
  return imported;
}
...
Hooks.once("ready", () => {
  ...
  game.settings.register(MODULE_ID, key, { name: "Aeria Core: версия сцен из книги городов", scope: "world", config: false, type: String, default: "" });
